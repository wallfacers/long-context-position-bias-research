...
\begin{itemize}
    ...
    \item complementary distribution-shift tests: controlled NoLiMa semantic retrieval and natural LongBench multi-document question answering (QA);
...
Lost in the Middle established controlled multi-document QA and key--value retrieval tests in which the location of relevant content changes while content remains fixed \citep{liu2024lost}. The RULER benchmark suite broadened synthetic long-context evaluation across retrieval, tracking, aggregation, and question answering \citep{hsieh2024ruler}. LongBench complements controlled tests with 21 natural and synthetic tasks, but does not by itself identify a causal position effect \citep{bai2024longbench}. NoLiMa removes direct lexical matches between question and evidence and shows that many models degrade well before their advertised context limit \citep{modarressi2025nolima}. Veseli et al. normalize input length by each model's maximum context and show that the familiar U-shape is strongest at relative lengths up to one half; nearer the limit, primacy weakens while recency remains, so a missing U-shape need not imply position invariance \citep{veseli2025shift}. CRE independently audits the reasoning-benchmark gap by jointly varying target position, filler content, and context length on GSM8K and the AI2 Reasoning Challenge (ARC)-Challenge; its model- and filler-dependent failures reinforce the need to report position-conditioned rather than aggregate scores \citep{zhang2026positional}. We combine strict within-group position control with natural transfer instead of treating either as sufficient alone.
...
\paragraph{NoLiMa semantic out-of-distribution (OOD) test.}
...
Tables~\ref{tab:main} and \ref{tab:factorial-contrasts} report seed-level means with Student-$t$ intervals over three independently trained seeds; the six factorial cells are themselves the ablation, each factor measured against a matched counterpart, and the audited compute ledger appears in the reproducibility section. The exploratory Qwen seed yielded \ExploratoryRuleAnswerMin--\ExploratoryRuleAnswerMax\ aggregate answer accuracy across the six 100-step treatments. That range alone obscured large position differences: paired answer supervision had only \ExploratoryPairedAnswerWorst\ worst-position accuracy and a \ExploratoryPairedAnswerGap\ gap, whereas independent exact-evidence supervision reached \ExploratoryIndependentEvidenceWorst\ worst-position accuracy with a \ExploratoryIndependentEvidenceGap\ gap. The frozen pilot therefore did not meet our formal all-cells saturation guard. We reported it as evidence that aggregate accuracy can conceal positional failures and kept it outside strict-primary means; it cannot identify a treatment that wins across cells and seeds.
...
In the historical fixed-100 partial-dose Qwen sensitivity analysis, no treatment consistently exceeded the fixed Base on worst-position accuracy; those observations motivated no treatment selection and could not support the strict pairing claim because the realized blocks were incomplete. Under the strict block-96 protocol (Table~\ref{tab:nolima}), three independently trained Qwen seeds placed every treatment below the untrained base on NoLiMa-Hard aggregate accuracy (13.0--15.7\% vs.\ 17.7\%) and on worst-position accuracy (1.7--5.6\% vs.\ 7.0\%). Within the factorial, evidence supervision retained a small but consistent direction: relative to answer-only targets, evidence-ID and exact-quote supervision improved worst-position accuracy by $+1.2$ $[+0.5,+1.9]$ and $+2.3$ $[+1.6,+3.0]$ points, the only Qwen contrasts whose seed-level intervals excluded zero in this suite, while position construction showed no reliable effect ($+1.4$ $[-1.7,+4.5]$ for pairing). The prospective Mistral replication was weaker in absolute terms (base 12.9\%) yet structurally different: both evidence-ID cells exceeded the base aggregate (17.9--18.4\%), evidence-ID minus answer improved worst position by $+6.4$ $[+0.9,+12.0]$ points, and paired answer-only training collapsed to 4.0\% aggregate with 0.0\% worst-position accuracy. Descriptively, the between-family difference of the evidence-ID-minus-answer effect was $+20.0$ points at the edge position and $-18.4$ points for answer consistency across positions, so the OOD behavior of nominally equivalent treatments was family-dependent. One format-level signature is worth separating from these accuracy patterns: answer-only cells scored exactly zero on the requested quote output despite untrained bases of 13.8--16.7\%, because answer-only supervision teaches the schema's quotation field to remain empty; quotation is therefore not merely unimproved but actively suppressed in those cells. Figure~\ref{fig:position} separates position construction from supervision granularity at each length using only strict adapters; consistent with the aggregate numbers, strict curves remain below their family base at most lengths.
...
No strict treatment improved over its family base on Qwen (Table~\ref{tab:longbench}; pooled token-F1 39.0--42.8 vs.\ 43.7 for base), and the Qwen factorial contrasts were indistinguishable from zero (e.g., evidence-ID minus answer $-1.4$ $[-7.9,+5.2]$). Mistral showed the opposite granularity pattern: both evidence-ID cells transferred above base (38.3 and 36.5 vs.\ 32.7), whereas both exact-quote cells fell roughly ten points below it (22.6 and 22.1), for an exact-evidence-minus-answer contrast of $-6.8$ $[-15.5,+2.0]$. Descriptively, the Mistral-minus-Qwen family difference was $+9.6$ points for evidence-ID supervision but $-7.8$ points for exact quotes. Because LongBench does not annotate movable gold evidence here, these scores establish natural transfer only: verifiability-oriented supervision does not uniformly help natural tasks and can hurt, with the sign depending on supervision granularity and family.
...
The exploratory Qwen representative separated three otherwise conflated failure modes. Oracle-short performance was near ceiling for Base and all four corner treatments, so answer generation from an isolated gold passage was not the dominant bottleneck. Exact-evidence treatments raised locate-only quotation from 6.0\% (Base) to 34.7--35.8\%, but this gain did not carry over to free-answer accuracy or to oracle-long evidence use under distractors. Paired answer supervision was the only Qwen corner that raised oracle-long accuracy (0.7\% to 2.0\%), yet its free-answer score still did not exceed Base, consistent with better use of already exposed evidence but unresolved localization. These representative-seed diagnostics are not multi-seed treatment estimates; Table~\ref{tab:mechanism} reports the clustered intervals.

In the regression checks (Table~\ref{tab:regression}), the Qwen MMLU reanalysis passed the frozen two-point noninferiority margin for every treatment after correctness was separated from JSON closure. IFEval showed no Holm-significant treatment difference, but none of its strict-prompt intervals established the same noninferiority guarantee. The Mistral regression checks mirrored its factorial instability: paired answer-only training lost roughly twenty MMLU points (34.3\% vs.\ 54.2\% base) and half its IFEval strict-prompt accuracy (21.3\% vs.\ 43.1\%), the same cell that collapsed on the rule and NoLiMa suites; the remaining Mistral cells passed MMLU noninferiority, and independent exact-quote training was the only Mistral cell that passed IFEval noninferiority. We therefore treat that cell's behavior as an optimization instability of one low-budget configuration rather than a family-level capability cost. We therefore report no detected instruction-following difference while retaining the uncertainty, rather than treating a nonsignificant test as equivalence. A treatment is interpreted as improving retrieval only if locate-only gains accompany free-task gains and oracle answer generation remains stable. The audited Mistral replication did not reproduce the Qwen decomposition. The Mistral base already answered from a trailing oracle block under long distractors (54.7\%), whereas the Qwen base barely did (0.7\%); independent answer-only training raised Mistral oracle-long accuracy to 96.7\% while leaving free answers near base (14.3\% vs.\ 12.9\%) and lowering oracle-short accuracy to 72.7\%; exact-quote supervision raised Mistral free answers to 16.8--24.2\% but left locate-only quotation at 6.2--13.9\%, below its 34.8\% base. The Qwen pattern---quotation taught without free-task gain---and the Mistral pattern---oracle use taught without localization---were qualitatively different; within this representative-seed diagnostic, the mechanism by which low-budget adaptation reshaped context use appeared family-dependent.

\begin{table*}[t]
\centering
\small
\caption{Representative-seed short-context regression checks. MMLU is full-test format-robust option accuracy and IFEval is official strict prompt accuracy. Noninferiority (NI) passes only when the paired 95\% interval for treatment minus Base lies entirely above the frozen $-2$ pp margin. JSON validity is audited separately and is not required for MMLU correctness.}
...
The two historical post-pilot fixed-100 Qwen NoLiMa catalogs agree on the qualitative failure shape. Answers change somewhere across the seven positions in roughly nine of ten eligible within-run groups, and edge-success/middle-failure groups are about four times as common as the reverse pattern. Invalid JSON and output-cap hits remain below one percent, so formatting failures cannot explain the low semantic answer accuracy. In matched Base--treatment comparisons, Base-only successes outnumber treatment-only successes in both seeds, consistent with the absence of a stable partial-dose treatment advantage. The strict catalogs (18 audited suites, both families) confirmed the repair at the failure level: on the rule suite, answers changed across the seven positions in 11.7\% (Qwen) and 20.6\% (Mistral) of eligible groups, far below the historical fixed-100 rate of roughly nine in ten, and on Qwen edge-success/middle-failure and middle-success/edge-failure groups occurred at nearly equal rates (9.5\% vs.\ 10.6\%), consistent with a flattened position curve, while Mistral retained an edge-directional asymmetry (18.5\% vs.\ 9.3\%). Formatting could not explain residual errors: invalid JSON and output-cap hits remained at or below 3.1\% in every strict suite. On NoLiMa the failure shape regressed: answers changed across positions in 90.2\% (Qwen) and 69.2\% (Mistral) of eligible groups, edge-success/middle-failure groups outnumbered the reverse roughly 3.6 to 1 in both families (44.6\% vs.\ 12.4\% and 38.2\% vs.\ 10.6\%), and matched comparisons tilted toward the base on Qwen (Base-only successes 5.1\% vs.\ treatment-only 1.8\%) while Mistral was balanced (4.6\% vs.\ 5.2\%). Identifier-fingerprinted per-case records reproduce these aggregate rates without containing benchmark text; we use them to illustrate, not to select, treatments. We keep the three denominator types separate and use these cases only to illustrate aggregate results, not to select a treatment or add post-hoc significance claims.

\section{Discussion}

The factorial design separates three conclusions that are often conflated. First, a flat in-distribution position curve can coexist with poor semantic or natural transfer. Second, answer-only correctness does not establish verifiable evidence use. Third, a balanced position histogram is not necessarily equivalent to showing the same underlying fact across positions. The strict results sharpen all three. The controlled-suite repair is real but factorially family-dependent: on Qwen, supervision granularity separates cells while position construction does not, whereas on Mistral the two factors interact so strongly that paired answer-only training is the worst cell and paired exact-quote training the best. The repair is also distribution-specific: on NoLiMa every Qwen cell remains below base and only Mistral evidence-ID cells edge past theirs, so the same cells that flatten the controlled curve leave semantic retrieval unchanged or degraded. And supervision granularity trades off across regimes rather than ordering monotonically: identifier supervision is the gentlest on NoLiMa and the only one above base on Mistral LongBench, whereas exact quotation maximizes controlled-suite quotation fidelity yet costs about ten LongBench points on Mistral.
...
NoLiMa data are used for non-commercial research under the Adobe Research License and are distributed through source pointers and generation hashes rather than relicensed. The LongBench repository is released under the Massachusetts Institute of Technology (MIT) License, while constituent datasets retain their original terms. The synthetic data contain generated facts rather than personal information. Public failure catalogs use one-way fingerprints in place of benchmark text and raw identifiers. We release no model-service credentials, machine passwords, or private paths.
...
All training and generation workloads are executed sequentially on one 32 GB RTX 5090-class instance rather than hidden behind multi-instance parallelism. The audited ledger records 188 sequential GPU events totaling 118.5 active GPU-hours (115.9 evaluation, 2.6 training) and a rental-cost lower bound of 329.42 Chinese yuan (CNY); the strict queue spans 2026-08-29 to 2026-09-04 wall-clock, including one operator-initiated shutdown and a lossless audit-gated resume. We report measured hardware and wall time so that compute can be compared, but do not estimate carbon emissions because the cloud vendor does not disclose machine power attribution, data-center location, or power usage effectiveness (PUE) for this rental.

\section{Conclusion}

We presented a matched factorial framework for attributing long-context position robustness to explicit cross-position pairing and supervision granularity, a position-equivalent evaluation protocol, complementary controlled and natural distribution-shift tests, and a reproducibility package with audited provenance. Under an audited strict block-complete protocol replicated across two model families, a single 96-step pass repaired controlled worst-position accuracy substantially, but the effective factor differed by family, and no treatment carried that repair to semantic retrieval without lexical overlap or to natural multi-document QA; supervision granularity even reversed sign across families on the latter. The strong form of the pairing hypothesis---that seeing the same fact across positions beats a matched independent histogram---was supported only as an interaction with verifiable supervision on one family. We release the full audit trail, adapters, and analysis code so that these boundary conditions on low-budget position-robustness training can be verified and extended.
...
Precision/runtime & BF16, TF32, gradient checkpointing, scaled-dot-product attention (SDPA) \\
