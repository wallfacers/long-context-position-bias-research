...
All training and generation workloads are executed sequentially on one 32 GB RTX 5090-class instance rather than hidden behind multi-instance parallelism. The audited ledger records 188 sequential GPU events totaling 118.5 active GPU-hours (115.9 evaluation, 2.6 training) and a rental-cost lower bound of 329.42 CNY; the strict queue spans 2026-08-29 to 2026-09-04 wall-clock, including one operator-initiated shutdown and a lossless audit-gated resume. We report measured hardware and wall time so that compute can be compared, but do not estimate carbon emissions because the cloud vendor does not disclose machine power attribution, data-center location, or PUE for this rental.

\section{Conclusion}

We presented a matched factorial framework for attributing long-context position robustness to explicit cross-position pairing and supervision granularity. Under an audited strict block-complete protocol replicated across two model families, a single 96-step pass repairs controlled worst-position accuracy substantially, but the effective factor differs by family, and no treatment carries that repair to semantic retrieval without lexical overlap or to natural multi-document QA; supervision granularity even reverses sign across families on the latter. The strong form of the pairing hypothesis---that seeing the same fact across positions beats a matched independent histogram---is supported only as an interaction with verifiable supervision on one family. We release the full audit trail, adapters, and analysis code so that these boundary conditions on low-budget position-robustness training can be verified and extended.
